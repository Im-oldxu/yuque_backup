...
MVP 采用一个仓库、三个运行组件、一个关系数据库、一个不可变内容目录和一个可读导出目录：
...
- 持久化：SQLite 保存业务数据、队列和检查点；本地内容目录保存正文快照、标准化 Markdown 和二进制资源；独立导出目录维护最新可读树、任务快照与清单；
...
| Markdown | markdown-it-py + markdownify | 统一六类文档的可移植 Markdown，并从可信 HTML 降级转换 | 不把运行时 HTML 当作唯一备份格式 |
| PDF | WeasyPrint + Pillow fallback | 从标准化 Markdown 按需生成 PDF，并在位图回退中保留中文字体 | 不把 PDF 作为版本文件或快照持久化 |
...
flowchart LR
    ...
    A -->|"鉴权后读取 Markdown/按需 PDF"| FS["内容目录：版本与资源"]
    A -->|"创建/取消任务"| DB
    K["worker：调度与备份，单实例"] --> DB
    K -->|"原子提交"| FS
    K -->|"重建 latest/任务快照"| EX["导出目录：可读 Markdown 树与清单"]
...
| `exports` | 六类文档 Markdown 标准化、历史回填、可读树/快照、清单、保留清理和按需 PDF |
...
1. 将原始响应、原始正文、标准化 `export.md`、标准化元数据、资源清单和清理后的 HTML 写入内容目录同一文件系统的临时区。
...
- 任务成功或部分成功后生成独立 Markdown 快照并重建 `latest`；快照沿用版本保留天数，`latest` 不按时间清理。
...
| `document_version` | 唯一 `document_id + content_hash`、远端版本、原始/Markdown/预览文件相对路径、完整性和来源任务 |
...
│   └── .tmp/{job_id}/
└── exports/
    ├── latest/{repository}/{toc-path}/{title}.md
    ├── snapshots/{timestamp-job}/
    │   ├── manifest.json
    │   └── {repository}/{toc-path}/{title}.md
    └── .tmp/
```
...
- 服务端继续用 nh3 allowlist 生成兼容离线 HTML；主阅读器通过鉴权端点读取标准化 Markdown，前端用 markdown-it 解析、DOMPurify 清理，并只使用本地 Shiki 语法和主题资源；
- 文章页不直接注入未清理的语雀正文；旧的离线预览端点仍通过无 `allow-scripts`、无 `allow-same-origin` 的 sandbox iframe 和严格 CSP 隔离；
...
- 版本提供鉴权 Markdown 读取、Markdown 下载和按需 PDF 下载；PDF 运行库不可用时返回稳定 `503 PDF_EXPORT_UNAVAILABLE`；
...
| 正文所有候选字段均为空 | 记录 `DOCUMENT_BODY_EMPTY`，不提交不可读版本 | 单项失败；最新成功指针保持 |
| PDF 运行库不可用 | 返回 `503 PDF_EXPORT_UNAVAILABLE`，不修改版本文件 | Markdown 与其他下载保持可用 |
...
- `api`：挂载 `/data/db`（读写）和 `/data/content`（只读），后端 Compose 默认仅暴露到宿主机 `127.0.0.1:8000`；PDF 直接从内容卷中的版本 Markdown 按需生成，无需读取导出卷；
- `worker`：挂载 `/data/db`、`/data/content` 和 `/data/exports`（读写），内容与导出各自使用同卷临时区，不提供网络入口；
- `api` 和 `worker` 使用同一主密钥、时区和数据根配置；
- Release 在同一 Tag/提交的普通 CI 成功后发布不可变 `vX.Y.Z` 多架构镜像、Linux amd64/arm64 与 Windows amd64 原生归档及 `checksums.txt`；Compose 为便于首次运行默认使用 `latest`，生产部署应在 `.env` 固定版本标签。预发布版本不更新 `latest`；容器以非 root 用户运行。
...
./data/exports  -> /data/exports   # 可读 Markdown latest、任务快照和清单
```
...
│   │   │                           # documents/assets/preview/exports/retention/settings
...
1. 已按 FastAPI + 单 worker + SQLite + 不可变内容目录 + 可读导出目录完成模块化单体实现；
2. SQLite 数据库必须放本地磁盘，NAS 只承载经过验证的内容与导出目录；
3. 前端在既有单应用底座上完成经过清理的 Markdown 阅读器、本地代码高亮、响应式文章大纲和备份状态；
4. 数据库迁移 head 为 `0002`，统一程序提供 `export` 子命令用于历史 Markdown 回填与导出树重建。
