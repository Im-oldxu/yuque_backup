...
MVP 采用一个仓库、三个运行服务、一个关系数据库和一个内容目录：

- 前端：严格基于 `tabtab-dev/ui` 的 `apps/admin` 二次开发，保留其 Vue 3、TypeScript、Vite Plus、Tailwind CSS v4、shadcn-vue/Reka UI、Pinia 和 Vue Router 4 底座；
...
- 部署：Docker Compose 运行 `web`、`api`、`worker`，不引入 Redis、PostgreSQL、Celery、S3 或 WebDAV；
...
7. **前端继承：** 复用 `tabtab-dev/ui` 的 Admin 应用、布局、组件、样式、路由和状态管理，不另建前端底座。

## 3. 调研结论与技术选型

### 3.1 tabtab-dev/ui 适配结论

截至 2026-07-23，上游默认分支 `main` 的 HEAD 为 `27a40c27bcd48c84035bff711133294025308ec2`。仓库提供完整 Admin SPA，技术栈为 Vue 3.5+、TypeScript 5.7+、Vite Plus 0.1.13、Tailwind CSS v4、shadcn-vue、Reka UI、Pinia、Vue Router 4 和 Vue I18n；要求 Node.js `>=20`、pnpm `10.32.1`。
...
- `apps/admin` 未内置固定的服务端数据方案，MVP 使用原生 `fetch` 封装 API 客户端即可，不额外引入 Axios 或另一套服务端状态框架；
- 上游只有少量提交、没有正式 Release，Vite Plus 仍为 `0.1.x`。MVP 必须固定上述提交和现有 `pnpm-lock.yaml`，不在开发期间无审查跟随 `main`。

**许可证风险：** README 标注 MIT，但仓库根目录的 `LICENSE` 链接当前返回 404，GitHub API 的 `license` 字段为 `null`。技术上没有阻塞，方案仍按要求基于该底座；但在复制、修改或分发其源码前，必须由上游补充有效许可证文本或取得版权所有者书面许可。该项是开发启动门禁，不能仅凭 README 的一行声明默认风险已关闭。
...
```mermaid
flowchart LR
    B["管理员浏览器"] -->|"同源 HTTPS/HTTP"| W["web：TabTab Admin + 反向代理"]
    W -->|"/api 与受控下载"| A["api：FastAPI，单进程"]
...
| 服务 | 副本 | 职责 | 禁止事项 |
| --- | --- | --- | --- |
| `web` | 1 | 构建并提供 TabTab Admin 静态文件；反向代理 `/api`；SPA history fallback | 不挂载数据库和内容目录 |
| `api` | 1 | 初始化、登录、配置、查询、手动触发、取消、预览和下载 | 不直接执行长时备份，不直接调用语雀 |
| `worker` | 1 | Cron、任务协调、语雀请求、资源下载、版本提交、保留清理和恢复租约 | 不提供公网端口，不允许多副本 |
| `migrate` | 一次性 | 容器启动前执行 Alembic 升级并退出 | 失败时禁止 API/worker 启动 |

`api` 和 `worker` 使用同一后端镜像、不同启动命令。MVP 固定单 API 进程和单 worker，避免 SQLite 锁竞争和 Cron 重复执行；未来若需要横向扩展，必须先更换数据库和任务协调方案，不属于本次设计。
...
| `assets` | URL 发现、SSRF 校验、流式下载、SHA-256 去重和引用 |
...
SQLite 中的 `queue_item` 是唯一队列事实来源，至少保存：任务、子任务、凭据、知识库、文档、类别、优先级、状态、尝试次数、`available_at`、`next_retry_at`、租约持有者、`lease_until`、幂等键和最后错误摘要。
...
6. 完整资源成功则版本为“完整”；正文可用但资源或特殊结构失败则为“部分成功”；正文不可提交则不更新最新指针。
...
| 场景 | 技术处理 | 状态影响 |
| --- | --- | --- |
| 429 | 解析 `Retry-After`；否则 60 秒起指数退避至 1 小时，持久化 `next_retry_at` | 对应凭据等待额度；其他凭据继续 |
...
- `web`：多阶段构建 TabTab Admin，运行时只包含静态产物和反向代理；默认对宿主机暴露一个端口；
- `migrate`：挂载 `/data/db`，执行数据库升级，成功后退出；
- `api`：挂载 `/data/db`（读写）和 `/data/content`（只读），不直接暴露宿主机端口；
- `worker`：挂载 `/data/db` 和 `/data/content`（读写），临时区固定为 `/data/content/.tmp`，不提供网络入口；
- `api` 和 `worker` 使用同一主密钥、时区和数据根配置；
- 所有镜像和依赖固定版本，不使用漂移的 `latest` 标签；容器以非 root 用户运行。
...
```text
yuque-backup/
├── frontend/                       # 固定提交导入的 tabtab-dev/ui monorepo
│   ├── apps/admin/src/
│   │   ├── api/                    # fetch 客户端、错误和类型映射
│   │   ├── router/                 # 基于会话和初始化状态的路由守卫
│   │   ├── stores/                 # 仅会话、设置和必要 UI 状态
│   │   ├── views/                  # 六类 MVP 页面
│   │   └── components/             # 业务组件，优先组合 @tabtab/ui
│   ├── packages/                   # 保留上游 UI/styles/utils
│   ├── pnpm-lock.yaml
│   └── UPSTREAM.md                 # 上游仓库、固定提交、许可证和本地修改
├── backend/
│   ├── pyproject.toml
│   ├── app/
│   │   ├── main.py                 # FastAPI 工厂和 lifespan
│   │   ├── api/                    # /api/v1 路由与依赖
...
| TabTab UI 成熟度 | 固定提交和 lockfile，不跟随主分支漂移 | 否 |
| TabTab UI 许可证 | README 称 MIT，但无 LICENSE 且 GitHub 未识别许可证；需补证或书面授权 | **是，阻塞复制/分发源码** |
...
请在进入详细 API 设计前确认以下三项：

1. 接受 FastAPI + 单 worker + SQLite + 本地内容目录的模块化单体方案；
2. 接受 SQLite 数据库必须放本地磁盘、NAS 只承载内容目录的约束；
3. 接受先解决 `tabtab-dev/ui` 许可证文本或书面授权，再导入其源码；前端技术栈和底座本身不变。

## 15. 调研来源

除另有说明，以下资料核验日期均为 2026-07-23：

1. [tabtab-dev/ui](https://github.com/tabtab-dev/ui)：前端技术栈、目录、Admin 能力、Node/pnpm 要求和 README 许可证声明。
2. [tabtab-dev/ui package.json](https://raw.githubusercontent.com/tabtab-dev/ui/main/package.json)：Vite Plus 0.1.13、pnpm 10.32.1 和 Node `>=20`。
3. [tabtab-dev/ui GitHub API](https://api.github.com/repos/tabtab-dev/ui)：默认分支、提交活跃度和 `license: null`。
