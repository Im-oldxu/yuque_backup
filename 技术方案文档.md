...
> 文档状态：已确认并完成 v1.0.0 实现\
> 版本：v1.0.0\
> 编写及技术调研日期：2026-07-23  
> 实现校准与验收日期：2026-07-24\
...
- 部署：仓库提供 `migrate`、`api`、`worker` 的后端 Docker Compose；前端构建为静态 SPA，由同源 Web 服务或反向代理提供，不引入 Redis、PostgreSQL、Celery、S3 或 WebDAV；
...
```mermaid
flowchart LR
    B["管理员浏览器"] -->|"同源 HTTPS/HTTP"| W["web：Vue SPA + 反向代理"]
...
| 服务 | 副本 | 职责 | 禁止事项 |
| --- | --- | --- | --- |
| `web` | 1 | 提供 Vue SPA 静态文件并反向代理 `/api`；Hash Router 无需业务路由 history fallback | 不挂载数据库和内容目录 |
...
- `web`：当前仓库通过 `frontend/` 独立安装、构建和发布静态产物；开发环境使用 Vite `/api` 代理，生产环境由部署者配置同源静态服务和反向代理；
